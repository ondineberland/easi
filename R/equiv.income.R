equiv.income <- function(object = object, log.exp_ref = log.exp_ref,
  ...
  tot0 <- temp$tot0
  if (interact) {
    y <- (log.exp_cur - tot0 + 1/2 * tot)/(1 - 1/2 * tot2)
  } else y <- (log.exp_cur - tot0 + 1/2 * tot)
  A <- y
  ...
  } else y2 <- (log.exp_cur - tot0 + 1/2 * tot)
